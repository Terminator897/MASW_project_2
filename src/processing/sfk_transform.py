...
def sfk_transform(data, dt, dr, f_min=5, f_max=80, v_min=50, v_max=500, 
    ...
    FTX = np.zeros((n_freqs, nt, nr), dtype=complex) # вот тут трехмерный куб создается. ниже заполняется, про который я в тг писала.
        # оси  это частота, время и номер трассы.
    
    for n in range(nr):
        trace_padded = np.zeros(new_nt)
        trace_padded[:nt] = data[n, :]
        st_result = stran(trace_padded, s_width=s_width) 
        FTX[:, :, n] = st_result[min_nf:max_nf+1, :nt]

    new_nr = nr * multi_nr
    k_axis = np.arange(new_nr) * (1 / (dr * new_nr))
    v_axis = np.linspace(v_min, v_max, new_nr) 
    
    fk2d = np.zeros((n_freqs, new_nr))
    vf2d = np.zeros((new_nr, n_freqs))

    for idx, f in enumerate(freq):
        TX = FTX[idx, :, :] # достаем срез из куба только для конкретной частоты. получаем 2д (время-номер трассы)
        # вообще наверное куб можно по частям обрабатывать, а не весь хранить. кажется его вообще можно не создавать, а
        # сделаь сначала S-преобразование всех трасс только для частоты f_1, расчет line_phase, расчет FK, затем переход к f_2 и тд
        line_phase = np.zeros((nr, nt), dtype=complex)
        for jj in range(nt): # это наверное тоже лучше векторизировать какнибуть 
            for ii in range(nr):
